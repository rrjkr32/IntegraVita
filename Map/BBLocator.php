...
?>
  <head>
	...
	function searchStores(location){
		...
		var b=$("#bg").val();//document.getElementById("medname").value;
		//hard coded the radius to 10 miles, if you get the value from a field if required
		var parameters = 'lat='+ location.lat() + '&lng=' + location.lng() + '&bg=' + b + '&qty=' + q;  
	...
	function displayStores(result){
		if (result.length > 0){
			for (i=0;i<result.length;i++){
				...
				var html = getEmbedHTML(i+1,result[i].bank_name,result[i].address,result[i].distance);
				...
				var msg = 'Location : ' + result[i].bank_name + '<br/> ';
        ...
        <div id="sidebar">
            <div class="row" style="background:#E3EDFA">
                ...
                <br>
							...
							<input type="number" id="qty" class="text" value="<?php if (isset($q)) echo $q ?>"/><br><br>
