<?php
	...
	$sql = sprintf("SELECT address, username, shop_name, lat, lng, ( 6371 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) 
	+ sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance FROM pharmacy_info HAVING distance < 10 ORDER BY distance LIMIT 0 , 20",
...
$rows = array();
	while($r = mysqli_fetch_assoc($result)) {
		$var=$r["username"];
		//echo $var;
		$sql2="select medicine_name,quantity from $var where medicine_name='$medname' and quantity>=$num";
		//echo $sql2;
